...
<div style="display:flex;flex-direction:column;gap:20px">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between">
        ...
        <button onclick="document.getElementById('modalTambah').style.display='flex'"
            style="display:flex;align-items:center;gap:8px;padding:10px 20px;border-radius:12px;border:none;cursor:pointer;color:#fff;font-size:13.5px;font-weight:700;background:linear-gradient(135deg,#3b82f6,#2563eb);box-shadow:0 4px 12px rgba(59,130,246,.3);transition:transform .15s"
    ...
    <div style="background:#fff;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,.05);overflow:hidden">
        ...
        <div style="padding:18px 24px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between">
            <div>
                ...
                <p style="font-size:12px;color:#94a3b8;margin-top:2px">{{ $users->count() }} akun ditemukan</p>
            </div>
        </div>
        ...
        <div style="padding:14px 24px;background:#f8fafc;border-top:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between">
            <p style="font-size:12.5px;color:#94a3b8;margin:0">Menampilkan {{ $users->count() }} data</p>
        </div>
...
<div id="modalEdit"
    ...
    <div style="background:#fff;border-radius:20px;width:100%;max-width:460px;box-shadow:0 20px 60px rgba(0,0,0,.15);overflow:hidden;animation:slideUp .2s ease">
        ...
        <form id="formEdit" method="POST" style="padding:24px 28px">
            @csrf
            @method('PUT')
            <div style="margin-bottom:16px">
                <label style="display:block;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#64748b;margin-bottom:7px">Nama Lengkap</label>
...
<script>
    ...
    function konfirmasiHapus(id, nama) {
        Swal.fire({
            title: 'Hapus Akun?',
            html: `Akun <strong>${nama}</strong> akan dihapus secara permanen dan tidak dapat dikembalikan.`,
            icon: 'warning',
    ...
    // ── Flash message sukses pakai SweetAlert2 Toast ────────────────
    @if(session('success'))
    Swal.fire({
        toast: true,
        position: 'top-end',
    ...
    // ── Buka modal edit & inject data user ──────────────────────────
    ...
    function filterTable(query) {
        ...
        document.querySelectorAll('#tableBody tr').forEach(row => {
            ...
            row.style.display = (name.includes(q) || email.includes(q) || nisn.includes(q))
                ? '' : 'none';
        });
    }
</script>
